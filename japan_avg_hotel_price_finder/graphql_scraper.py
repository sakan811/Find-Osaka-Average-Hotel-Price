...
def get_graphql_query(city: str, check_in: str, check_out: str, page_offset: int = 0) -> dict:
    """
    ...
    return {
      ...
      "variables": {
        "input": {
          ...
          "filters": {"selectedFilters": "ht_id=204"},
          "flexibleDatesConfig": {
            "broadDatesCalendar": {
              "checkinMonths": [],
              "los": [],
...
def check_info(
        ...
        entered_selected_currency: str) -> tuple:
    """
    ...
    :return: Tuple of total page number, city, check-in date, check-out date, and currency.
    """
    if response.status_code == 200:
        ...
        if entered_city != city_data:
            ...
            raise SystemExit(f"Error City not match: {entered_city} != {city_data}")
        if entered_check_in != check_in_data:
            logger.error(f"Error Check-in not match: {entered_check_in} != {check_in_data}")
            raise SystemExit(f"Error Check-in not match: {entered_check_in} != {check_in_data}")
        if entered_check_out != check_out_data:
            logger.error(f"Error Check-out not match: {entered_check_out} != {check_out_data}")
            raise SystemExit(f"Error Check-out not match: {entered_check_out} != {check_out_data}")
        if entered_selected_currency != currency_data:
            logger.error(f"Error Currency not match: {entered_selected_currency} != {currency_data}")
            raise SystemExit(f"Error Currency not match: {entered_selected_currency} != {currency_data}")

        return total_page_num, city_data, check_in_data, check_out_data, currency_data
    else:
        logger.error(f"Error: {response.status_code}")


def scrape_graphql(city: str = None, check_in: str = None, check_out: str = None, selected_currency: str = None) -> pd.DataFrame:
    """
    ...
    if check_in and check_out and selected_currency:
        ...
        total_page_num, city, check_in, check_out, currency = check_info(response, city, check_in, check_out, selected_currency)
        logger.debug(f"Total page number: {total_page_num}")
        logger.debug(f"City: {city}")
        logger.debug(f"Check-in date: {check_in}")
        logger.debug(f"Check-out date: {check_out}")
        logger.debug(f"Currency: {currency}")

        df_list = []
        logger.info("Scraping data from GraphQL endpoint...")
        for offset in range(0, total_page_num, 100):
            graphql_query = get_graphql_query(city=city, check_in=check_in, check_out=check_out, page_offset=offset)
            response = requests.post(url, headers=headers, json=graphql_query)
