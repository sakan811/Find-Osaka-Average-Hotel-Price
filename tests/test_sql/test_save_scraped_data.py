...
def test_save_scraped_data_non_empty(mock_migrate, mock_logger, sample_dataframe, tmp_path):
...
@patch('japan_avg_hotel_price_finder.sql.save_to_db.main_logger')
@patch('japan_avg_hotel_price_finder.sql.save_to_db.migrate_data_to_sqlite')
def test_save_scraped_data_empty(mock_migrate, mock_logger, empty_dataframe, tmp_path):
    db_path = str(tmp_path / 'test_db.sqlite')
